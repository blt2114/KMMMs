...
def main():
    ...
    parser.add_argument('-batch_size', dest="batch_size", action="store",
                        help="batch_size for svi",
                        type=int, default=1024)
    parser.add_argument('-tau', dest="tau", action="store",
                        help="delay parameter", type=float,
                        default=10.0)
    parser.add_argument('-kappa', dest="kappa", action="store",
                        help="forget rate", type=float,
                        default=0.5)
    ...
    log_p_longer = args.log_p_longer*2.0/int(4**K/batch_size)
    kappa = args.kappa
    tau = args.tau
    ...
    if restart_iter == 0:
        motifs = inference_tools.initialize_motifs(n_motifs, f=f, pi_mag=pi_mag,
                ...
                max_len=max_len, sym_motifs=sym_motifs, K= K, variational=True, 
                phi=phi, tau=tau, kappa=kappa)
    else:
        motifs = inference_tools.load_motifs_from_file(n_motifs, f=f, pi_mag=pi_mag,
                log_p_longer=log_p_longer, seq_len=min_len, min_len=min_len,
                max_len=max_len, sym_motifs=sym_motifs, K= K,
                out_dir=output_dir, restart_iter=restart_iter, variational=True,
                phi=phi, tau=tau, kappa=kappa)

    ### Load Kmer table
    tools.log("loading in kmer table", f)
    kmmm = tools.KMMM(kmer_table_fn, motifs, alpha_bg=alpha_bg,
            alpha_m=alpha_m, K=K, variational=True, kappa=kappa,tau=tau)
    ...
    ### Begin inference by Stochastic variational inference
    tools.log("Beginning inference", f)
    for i in range(restart_iter+1, n_iter):
        ...
        f = open(log_fn, 'a')
        inference_tools.log_state(phi, motifs, kmmm, f=f,stochastic=True)
        kmmm.update_gamma()
        f.flush()
    ...
    f = open(log_fn, 'a')
    tools.log("inference concluded", f)
    if f != sys.stdout:
        f.close()
